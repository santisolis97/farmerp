...
module.exports = function sendReport(rptname, datos, res, orientation) {
    ...
    datos._pathBaseImages = path.resolve("reports")
    datos._today = _today
    datos._now = _now

    ejs.renderFile(path.join(__dirname, '..', 'reports', rptname, rptname + ".ejs"), datos, (err, data) => {
        if (err) {
            res.send(err);
        } else {
            let options = {
            ...
            pdf.create(data, options).toFile("./reports/temp/" + rptname + ".pdf", function (err, data) {
                if (err) {
                    res.send(err);
                } else {
